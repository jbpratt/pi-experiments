...
- [`extensions/gh-search`](extensions/gh-search) — bounded, parsed GitHub code/repo/issue/PR/commit search and single-file reads via the `gh` CLI (`gh_search`, `gh_read_file`), with a non-blocking nudge away from `bash` + `curl` against GitHub's raw/API hosts.
