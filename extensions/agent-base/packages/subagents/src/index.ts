 ...
 *
 * Starts a persistent Pi session in a tmux pane for each invocation,
 * giving it an isolated context window and a visible interactive workspace.
 *
 ...
 *
 * Uses agent-base A2A coordination for delivery, status, results, and cancellation.
 */
...
import {
	type CoordinatedTaskSnapshot,
	isTerminalTaskState,
	requestCoordinationApi,
} from "./coordination.ts";
import { closeTmuxPane, currentTmuxPane, launchTmuxWorker } from "./tmux-worker.ts";
...
interface SingleResult {
	...
	paneId?: string;
	sessionId?: string;
	taskState?: CoordinatedTaskSnapshot["state"];
	step?: number;
}
...
function getSuccessfulResultOutput(result: SingleResult): string {
...
function getDisplayItems(messages: Message[], output?: string): DisplayItem[] {
...
async function runSingleAgent(
	...
	const emitUpdate = (text: string) => onUpdate?.({
	...
	try {
		...
		if (signal?.aborted) {
			if (taskCreated && currentResult.taskId) {
				...
				await api?.cancel(currentResult.taskId, AbortSignal.timeout(2_000)).catch(() => undefined);
			} else if (currentResult.paneId) {
				await closeTmuxPane(currentResult.paneId);
			}
			throw new Error("Subagent was aborted.");
		}
		if (!taskCreated && currentResult.paneId) await closeTmuxPane(currentResult.paneId);
...
function applySnapshot(result: SingleResult, snapshot: CoordinatedTaskSnapshot): void {
...
function safeCoordinationApi(pi: ExtensionAPI) {
	try {
...
function safeMessage(error: unknown): string {
	if (!(error instanceof Error)) return "Persistent subagent failed.";
